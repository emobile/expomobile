class FaceToFacesController < ApplicationController
  ...
  def index
    @face_to_faces = FaceToFace.order('id DESC').paginate(:per_page => 10, :page => params[:page])
  ...
  def get_interviewee
    ...
    @interviewee = Exhibitor.where(:name => params[:name]).select([:social_reason, :job]).first if @interviewee.blank?
